...
const authorize = (requiredPermissions = []) => {
  ...
  return async (req, res, next) => {
      console.log('[RBAC] req.userId=', req.user.id, 'requiredPermissions=', requiredPermissions);
    try {
      ...
      if (requiredPermissions.length > 0) {
        ...
        if (!hasAll) return res.status(StatusCodes.FORBIDDEN).json({ message: 'Forbidden' });
      }
